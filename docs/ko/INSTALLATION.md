...
- 현재 릴리스 후보에서는 Localify 1.50.0의 `localify.dll`과 유효한 루트 `config.json`
...
5. 설치를 누릅니다. 설치기는 전체 payload hash와 manifest를 먼저 검증한 뒤 파일을 백업·설치하고 `config.json.externalDlls`에 UmaVR DLL을 병합합니다.
6. `vrmod/tools/UmaVR.Configurator.exe`에서 설정을 확인한 뒤 DMM 런처로 게임을 시작합니다.
...
저장소는 초기 private 상태이며 프로그램에 GitHub 자격 증명을 포함하지 않습니다. 따라서 일반 사용자의 자동 업데이트는 안전한 공개/배포 방식이 결정될 때까지 사용할 수 없습니다.
...
- 설치가 거부되면 게임 프로세스, 게임 폴더, `localify.dll`, `config.json`을 확인하십시오.
